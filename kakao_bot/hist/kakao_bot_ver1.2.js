    ...
    if (currentErrorCount >= MAX_ERROR_COUNT) {
        ...
        if (db[room].errorCount === MAX_ERROR_COUNT) {
            ...
            if (msg.startsWith("/")) {
                ...
                if (query2) {
                    const aiResult2 = searchAI(query2);
...
function response(room, msg, sender, isGroupChat, replier, imageDB, packageName) {
    ...
    if (msg.startsWith('/')) {
        ...
        if (msg.startsWith("/날씨 ")) {
            ...
            const allUsers = Utils.getUsers(room);
            if (!allUsers) {
                replier.reply("방 참여자 정보를 불러올 수 없습니다.");
                return;
            }
            const targetUsers = allUsers.filter(user => user.name === targetName);
            if (targetUsers.length === 0) {
                replier.reply(`'${targetName}'님은 현재 이 방에 없거나, 이름을 잘못 입력하셨습니다.`);
                return;
            }
            const logs = Utils.getChatLogs(room, 5000, true);
            if (!logs || logs.length === 0) {
                replier.reply("채팅 기록이 없거나 불러올 수 없습니다.");
                return;
            }
            const userInfo = {};
            ...
            const totalChatCount = logs.length;
            // 5. 로그를 분석하여 각 유저의 정보 업데이트
            logs.forEach(log => {
                // 이 로그의 발신자(log.profileId)가 우리가 찾는 유저 중 한 명이라면
                if (userInfo[log.profileId]) {
                    userInfo[log.profileId].chatCount++;
                    userInfo[log.profileId].lastChatTimestamp = log.time;
                }
            });
            // 6. 결과 메시지 생성
            let resultMessage = [`[🔍 '${targetName}'님 검색 결과 (${targetUsers.length}명)]`, ''];
            Object.values(userInfo).forEach((user, index) => {
                // 채팅 기록이 없는 유저는 건너뛰기
                if (user.chatCount === 0) {
                    resultMessage.push(`[👤 ${user.name} (${index + 1})]\n\n채팅 기록이 없습니다.`);
                    return;
                }
                const contribution = ((user.chatCount / totalChatCount) * 100).toFixed(2);
                const lastChatDate = new Date(user.lastChatTimestamp);
                const formattedLastChat = lastChatDate.toLocaleString("ko-KR");
                const userBlock = [`[👤 ${user.name} (${index + 1})]`, ``, `💬 마지막 채팅:`, `  ${formattedLastChat}`, ``, `📈 채팅 기여도:`, `  - 채팅 수: ${user.chatCount}회`, `  - 방 전체 지분: ${contribution}% (총 ${totalChatCount}개 중)`].join('\n');
                resultMessage.push(userBlock);
            });
            // 각 유저 정보 사이에 구분선 추가
