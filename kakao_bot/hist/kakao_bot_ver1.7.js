 ...
 * * * [ver 1.3.8.] 20250721 게임 1개(초성게임) 추가     (##12)
 */
...
const GAME_POINT = 5;      //     게임 승리 시 획득 포인트
const S_GAME_POINT = 7;    // 특수 게임 승리 시 획득 포인트
...
const POINT_GRADE_5 = 250; // 사용자 포인트 5단계
const POINT_GRADE_6 = 500; // 사용자 포인트 6단계
const POINT_GRADE_7 = 800; // 사용자 포인트 7단계
const POINT_GRADE_8 = 1200;// 사용자 포인트 8단계
const POINT_GRADE_9 = 2000;// 사용자 포인트 9단계
const DELIMITER = ";";
...
const sentenceQuizList = [
    ...
    "동해물과__백두산이__마르고__닳도록",
    "하나님이__보우하사__우리나라__만세",
...
// [초성퀴즈] 문제 목록 (정답, 힌트)
//const chosungQuizList = [
//    { answer: "사과", hint: "원숭이 엉덩이는 빨개, 빨간 건?" },
//    { answer: "카카오톡", hint: "한국인이 가장 많이 쓰는 메신저" },
//    { answer: "아이스아메리카노", hint: "얼죽아" },
//    { answer: "인공지능", hint: "스스로 학습하는 컴퓨터 프로그램" },
//    { answer: "바나나", hint: "노랗고 긴 과일" }
//    { answer: "자전거", hint: "두 개의 바퀴가 있고, 페달을 밟아 움직이는 교통수단입니다." },
//    { answer: "노트북", hint: "휴대할 수 있는 개인용 컴퓨터입니다." },
//    { answer: "아파트", hint: "여러 세대가 함께 거주하는 높은 공동주택입니다." },
//    { answer: "냉장고", hint: "음식을 차갑게 보관하여 신선도를 유지하는 가전제품입니다." },
//    { answer: "소방관", hint: "불을 끄고 위험에 처한 사람들을 구하는 직업입니다." }
//];
...
const chosungQuizList = [
    ...
    { answer: "다람쥐", hint: "" },
    { answer: "도토리", hint: "" },
    { answer: "가는 말이 고아야 오는 말이 곱다", hint: "속담" },
    { answer: "태극기 휘날리며", hint: "영화" },
    { answer: "나홀로집에", hint: "영화" },
    { answer: "과속스캔들", hint: "영화" },
    { answer: "아브라카다브라", hint: "음악" },
    { answer: "기억을 걷는 시간", hint: "음악" },
    { answer: "그립고 그립고 그립다", hint: "음악" },
    { answer: "집에 가자", hint: "음악" },
    { answer: "어떻게 이별까지 사랑하겠어, 널 사랑하는 거지", hint: "음악" },
    { answer: "별에서 온 그대", hint: "드라마" },
    { answer: "이태원클라스", hint: "드라마" },
    { answer: "도깨비", hint: "드라마" },
    { answer: "나의아저씨", hint: "드라마" },
    { answer: "카카오톡", hint: "" },
    { answer: "토끼", hint: "" },
    { answer: "냉장고", hint: "" },
    { answer: "물고기", hint: "" },
    { answer: "커피", hint: "음료" },
    { answer: "핸드폰", hint: "전자기기" },
    { answer: "기차", hint: "교통" },
    { answer: "치즈", hint: "" },
    { answer: "양말", hint: "" },
    { answer: "텔레비전", hint: "전자기기" },
    { answer: "달력", hint: "" },
    { answer: "노트북", hint: "전자기기" },
    { answer: "얼음", hint: "" },
    { answer: "고양이", hint: "동물" },
    { answer: "호랑이", hint: "동물" },
    { answer: "우산", hint: "물건" },
...
function startChosungGame(room, replier) {
    ...
    ongoingGames[room] = { type: 'chosung', answer: quizData.answer };
...
function startSpaceGame(room, replier) {
    const randomIndex = Math.floor(Math.random() * sentenceQuizList.length);
    const quiz = sentenceQuizList[randomIndex];
    const answer = quiz.replace(/ /g, ''); // 정답: '__'를 공백으로 변경
...
const defaultName = [
      "손흔드는 라이언", "기뻐하는 라이언", "화난 라이언", "좌절하는 라이언", "귀여운 라이언", "운동하는 라이언", "부끄러운 라이언", "눈물바다에 빠진 라이언", "생각하는 라이언", "하트뿅뿅 라이언", "벌 서는 라이언", "졸린 라이언", "베게를 부비적대는 라이언", "블럭을 무너트리는 라이언", "마이크를 든 라이언"
    ...
    */
    ...
    if (msg.startsWith('/')) {
        ...
        // ==================== 출석체크 관련 함수 =================== ##7
            ...
            if (msg.trim() === "/검색 테스트봇"
            ...
            else if (msg.trim() === "/검색 놀아줘" || msg.trim() === "/검색 지루해") {
            ...
            else if (msg.trim() === "/검색 귀여워" || msg.trim() === "/검색 귀엽다구" || msg.trim() === "/검색 귀여워") {
                replier.reply("(˶' ᵕ ' 🫶) 감사해요 헤헤");
                return;
            }
            else if (msg.trim() === "/검색 거울아 거울아 세상에서 누가 제일 예쁘니?" || msg.trim() === "/검색 거울아 거울아") {
                replier.reply("(｡•̀ᴗ -) ✧ 저요 데헷)");
                return;
            }
                ...
                try {
                    ...
                    const allUsers = JSON.parse(FileStream.read(DB_PATH));
                    const pointRanking = [allUsers] // 배열 복사
                        .sort((a, b) => b.points - a.points)
                        .slice(0, RANKING_LIMIT);

                    let pointResult = '\n\n🏆 포인트 랭킹 TOP ' + RANKING_LIMIT + '🏆\n';
                    pointResult += "────────────────────\n";
                    pointRanking.forEach((user, index) => {
                        pointResult += (index + 1) + '위: ' + user.nickname + ' (' + user.points + 'P)\n';
                    });
                    ...
                    let streakResult = '\n🗓️ 연속 출석 랭킹 TOP ' + RANKING_LIMIT + '🗓️\n';
                    streakResult += "────────────────────\n";
                    streakRanking.forEach((user, index) => {
                        streakResult += (index + 1) + '위: ' + user.nickname + ' (' + user.attendanceStreak + '일)\n';
                    });

                    replier.reply(result + pointResult.trim() + "\n" + streakResult.trim());
                } catch (e) {
            ...
            if (roomGradeDB[room] >= 3 || userPoint >= POINT_GRADE_3) { // 3등급 이상
                ...
                if (msg.startsWith("/뉴스")) {
                    ...
                    const GAME_SETTINGS_PATH = 
                    let settings = loadData(GAME_SETTINGS_PATH);
                    ...
                    GAME_LIST.forEach(game => {
                        // 설정이 없으면 기본 가중치 1, 설정값이 0 이상이면 그 값을 사용
                        const weight = roomSettings[game] === undefined ? 1 : roomSettings[game];
                        if (weight > 0) {
                            weightedGames.push({ name: game, weight: weight });
                            totalWeight += weight;
                        }
                    });
                    ...
                    } else if (Math.random() >= 0.1 && Math.random() < 0.4) {
                ...
                if (roomGradeDB[room] >= 4 || userPoint >= POINT_GRADE_4) {
                    if (msg.startsWith("/맛집 ")) {
                        ...
                        // 지원하지 않는 명령어는 무시
                        if (!configs[command]) return;

                        let config = configs[command];
    ...
    // 현재 진행 중인 게임이 있을 경우, 모든 메시지를 정답으로 간주하고 확인
        ...
        // 정답을 맞혔을 경우 공통 처리
        if (correct) {
            result_cont += '(\n획득 포인트 +' + game.type ? S_GAME_POINT + '(S)': GAME_POINT
                         + ', 현재: ' + setUserPoint(room, sender, game.type ? S_GAME_POINT : GAME_POINT) + '점)';
