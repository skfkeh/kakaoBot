...
function loadAndParseCSV(filePath) {
    try {
        const csvData = FileStream.read(filePath); // 파일 경로는 전역 변수로 관리
        if (!csvData) {
        ...
        lines.forEach(line => {
            // 정규표현식을 사용하여 따옴표 안의 쉼표는 무시하고 필드를 분리합니다.
            const parts = line.match(/(".*?"|[^",]+)(?=\s*,|\s*$)/g) || [];

            // 각 필드의 앞뒤에 붙어있을 수 있는 따옴표를 제거하고 공백을 정리합니다.
            const cleanedParts = parts.map(p => p.replace(/^"|"$/g, '').trim());

            if (cleanedParts.length < 6) return; // 컬럼 수가 부족하면 무시

            let [qNum, qText, qContext, qAnswer, optNum, optText] = cleanedParts;
            ...
            if (!quizzes[qNum]) {
                quizzes[qNum] = {
                    question: qText.replace(/""/g, '"'), // CSV 내부의 ""는 "로 변환
                    context: qContext.replace(/""/g, '"'),
                    answer: qAnswer,
                    options: []
                };
            }
...
const chosungQuizList = [
    ...
    { answer: "다람쥐", hint: "" },
    { answer: "도토리", hint: "" },
    { answer: "가는 말이 고와야 오는 말이 곱다", hint: "속담" },
...
function startChosungGame(room, replier) {
    const chosungQuizList = loadDatabase(DB_BASE_PATH + chosung);
...
function startSqldGame(room, replier) {
    const parsedQuizzes = loadAndParseCSV(DB_BASE_PATH + SQLD_QUIZ);
    if (!parsedQuizzes || Object.keys(parsedQuizzes).length === 0) {
        replier.reply("SQLD 문제 파일을 불러오는 데 실패했습니다. 다른 게임을 시작합니다.");
        ...
        // ==================== 출석체크 관련 함수 =================== ##7
            ...
            if (roomGradeDB[room] >= 3 || userPoint >= POINT_GRADE_3) { // 3등급 이상
                ...
                if (msg.startsWith("/뉴스")) {
                    ...
                    // 방 게임 세팅비율
                    let roomSetting = room.includes("sqld") ? [0.76, 0.82, 0.88, 0.94] : [0, 0.7, 0.9, 0.95];
