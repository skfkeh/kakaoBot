...
const CHOSUNG_QUIZ = "quiz/chosung_test.json";
const NONSENSE_QUIZ = "quiz/nonsense_test.json";
...
// 게임 목록 (관리 및 설정에 사용)
const GAME_LIST = ['업다운', '문장', '초성', 'sqld', '넌센스'];

// 형식: { "방이름": { "유저해시": { type: 'chosung', answer: '단어' } } }
const quizAddState = {};

function gameDelete(dbName, replier, wordToDelete) {
...
function gameMaker(dbName, replier, answer, hint) {
    ...
    try {
        ...
        games.push({ answer: answer, hint: hint });
    ...
    try {
        ...
        if (msg.startsWith('/')) {
            ...
            //=================== json 파일보기 ==================== ##3
                ...
                var fileNameChg = "";
                if (fileName === "채팅") {
                    fileNameChg = escapeJsonString(mode) + "chat_log.json";
                } else if (fileName === "에러") {
            ...
            //=================== 게임등록 명령어 ================== ##6
            else if (msg.startsWith("/초성")) {
                //let gameList = JSON.stringify(FileStream.read(DB_BASE_PATH + CHOSUNG_QUIZ) || "[]");
                ...
                const content = msg.substring(8).trim();
                ...
                replier.reply('✅ 초성퀴즈 [' + answer + ' | 힌트: ' + hint + ']가 성공적으로 등록되었습니다!');
                return;
            }
                ...
                gameDelete(CHOSUNG_QUIZ, replier, wordToDelete);

                return; // 명령어 처리 후 종료
            }
            else if (msg.startsWith("/넌센스")) {
                ...
                const quizList = loadData(NONSENSE_QUIZ_PATH, true);
                ...
                saveData(DB_BASE_PATH + NONSENSE_QUIZ, quizList);
            ...
            //=================== 공지 등록/삭제 기능 ============== ##7
                ...
                const noticeContent = msg.substring(7 + roomName.length).trim();
                if (!noticeContent) {
                    replier.reply("등록할 공지 내용을 입력해주세요.");
                    return;
                }

                replier.reply(roomName + "///" + noticeContent);
            ...
            //=================== 메세지 보내기 =================== ##8
                ...
                let roomName = msg.split(' ')[1];
                let msgContext = msg.substring(4 + roomName.length).trim();
                if (replyRoom(roomName, msgContext, false)) {
